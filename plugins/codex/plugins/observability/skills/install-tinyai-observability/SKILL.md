...
Then repair any legacy TinyAI MCP block in `~/.codex/config.toml` and remove
older TinyAI Observability cache versions. Older local installs may keep a
hard-coded runtime path even after `codex plugin add` upgrades the marketplace
plugin. If the hard-coded path points to a removed cache directory, Codex will
show the plugin as enabled but the MCP process will not start, so no Codex
session data is uploaded.
...
for child in plugin_root.iterdir():
    ...
    try:
        ...
        print(f"Could not remove old TinyAI Codex cache {child}: {exc}")
if removed:
    print("Removed old TinyAI Codex cache versions:")
    for path in removed:
        print(f"  {path}")
PY
```
...
TINYAI_OBS_CODEX_USER_ID="<confirmed-name>"
EOF
...
user_id = env("TINYAI_OBS_CODEX_USER_ID") or env("TINYAI_OBS_USER_ID") or username
