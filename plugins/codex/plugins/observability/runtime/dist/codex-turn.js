...
function latestTurnIndex(snapshot) {
    const indexes = [
...
function lastValue(items, predicate) {
    return [...(items || [])].reverse().find(predicate);
}
function fallbackId(kind, sessionId, signature) {
    return `codex_${kind}_${stableEventId(`${kind}:${sessionId || "unknown"}:${signature}`).slice(0, 24)}`;
}
function codexLatestTurnBoundary(messages) {
    for (let index = messages.length - 1; index >= 0; index -= 1) {
        const message = messages[index];
        if (message?.role === "user")
            return { index, sequence: message.sequence };
    }
    return undefined;
}
function isAfterBoundary(item, boundarySequence) {
    if (boundarySequence === undefined)
        return true;
    return typeof item.sequence === "number" && item.sequence >= boundarySequence;
}
function scopedCodexTurn(snapshot, options) {
...
export function codexSnapshotSignature(snapshot) {
    return stableEventId(JSON.stringify({
...
export function codexTurnSnapshotPayload(snapshot, options = {}) {
    ...
    return {
        ...
        capture_limitations: "Captured from locally persisted Codex session data. Visible reasoning and tool output are included only when present in the log; hidden model reasoning is not available."
    };
}
export function buildCodexTurnSnapshotEvent(snapshot, options = {}) {
