...
export function classifySpecPath(filePath: string): SpecClassification {
...
function inferMatchedBy(text: string): string[] {
  const hits: string[] = [];
  if (/keywords?/i.test(text)) hits.push("keywords");
  if (/related[_-]?code/i.test(text)) hits.push("related_code");
  if (/modules?/i.test(text)) hits.push("module");
  if (/tags?/i.test(text)) hits.push("tags");
  return hits;
}

async function walk(root: string, maxFiles = 300): Promise<string[]> {
  ...
  async function visit(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      else if (/\.(md|ya?ml)$/i.test(entry.name)) results.push(path);
    }
  }
  await visit(root);
  return results;
...
function inferContentMatchedBy(content: string, terms: string[]): string[] {
  ...
  for (const [name, pattern] of fields) {
    ...
    if (terms.some((term) => window.includes(term))) hits.add(name);
  }
  if (hits.size === 0 && terms.some((term) => lower.includes(term))) hits.add("body");
  return [...hits];
}

export async function searchSpecs(workspacePath: string, query: string): Promise<Array<{ path: string; excerpt: string; matched_by: string[] }>> {
  const roots = [
  ...
  const scored: Array<{ path: string; excerpt: string; score: number; matched_by: string[] }> = [];
...
export async function readSpec(workspacePath: string, specPath: string): Promise<{ path: string; content: string; classification: SpecClassification }> {
