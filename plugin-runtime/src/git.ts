...
function gitMaxBufferBytes(): number {
...
function isSensitiveDiffPath(filePath: string): boolean {
...
function decodeGitQuotedPath(raw: string): string {
  ...
  if (!/\\(?:[0-7]{3}|[abfnrtv\\"'])/.test(inner)) return trimmed;
...
function safeDiffLineText(filePath: string, text: string, includeText: boolean): { text: string; redacted: boolean } {
  if (!includeText) return { text: "[text not stored]", redacted: true };
  if (isSensitiveDiffPath(filePath)) return { text: "[REDACTED:SENSITIVE_FILE]", redacted: true };
  const redacted = redactText(text, { allowFullConversationText: true });
  return { text: redacted, redacted: redacted !== text };
}

function parseUnifiedDiffDetails(diff: string, options: { includeText?: boolean; maxFiles?: number; maxLinesPerFile?: number } = {}): GitDiffDetails {
  const includeText = options.includeText ?? true;
  const maxFiles = options.maxFiles ?? 30;
  const maxLinesPerFile = options.maxLinesPerFile ?? 240;
  const files: DiffDetailFile[] = [];
  let currentFile: DiffDetailFile | undefined;
  ...
  function pushFile(file: DiffDetailFile | undefined) {
    if (!file) return;
  ...
  function addLine(lineType: DiffDetailLine["line_type"], rawText: string) {
    ...
    const detail: DiffDetailLine | undefined = captureLine
      ? (() => {
          const display = safeDiffLineText(currentFile.file_path, rawText, includeText);
          const captured: DiffDetailLine = {
            line_type: lineType,
            text: display.text,
            text_hash: lineHash(currentFile.file_path, rawText)
          };
          if (display.redacted) captured.redacted = true;
          return captured;
        })()
      : undefined;
    if (lineType === "added") {
      if (detail) detail.new_line = newLine;
      currentFile.lines_added += 1;
      totalAdded += 1;
      newLine += 1;
    } else if (lineType === "removed") {
      if (detail) detail.old_line = oldLine;
    ...
    if (detail) currentHunk.lines.push(detail);
  }
...
export async function currentDiffDetails(
  workspacePath: string,
  options: { includeText?: boolean; staged?: boolean; maxFiles?: number; maxLinesPerFile?: number; paths?: string[]; includeUntracked?: boolean } = {}
...
function normalizePathspecs(paths: string[] | undefined): string[] {
  ...
  for (const raw of paths || []) {
    ...
    if (value && value.length < 1000) output.add(value);
  }
  return [...output].slice(0, 50);
}

async function untrackedFilesDiff(workspacePath: string, pathspecs: string[], includeText: boolean): Promise<string> {
  let raw = "";
  try {
  ...
  for (const filePath of files) {
    if (isSensitiveDiffPath(filePath)) continue;
    try {
...
export async function installGitHooks(workspacePath: string, options: { tool: string; collectorUrl?: string; fallbackUrls?: string[]; token?: string; pluginVersion?: string; envFile?: string }):
  ...
  const postCommit = managedHookBlock(
...
function managedHookBlock(description: string, command: string): string {
...
async function writeManagedHook(hookPath: string, block: string): Promise<void> {
  let existing = "";
  try {
...
async function removeManagedHook(hookPath: string): Promise<void> {
  let existing = "";
  try {
    existing = await readFile(hookPath, "utf8");
  } catch {
  ...
  if (!TINYAI_HOOK_RE.test(existing) && !isLegacyTinyAiHook(existing)) return;
...
function isLegacyTinyAiHook(value: string): boolean {
